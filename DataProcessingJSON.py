...
def main(input_path):
    ...
    try:
        ...
        # list_of_dfs = [pd.read_json(input_path + '/' + filename) for filename in file_list]

        #for df, filename in zip(list_of_dfs, file_list):
    ...
    try:
     ...
     # log the unique value count for the column in chicago_collision dataframe

    #logger.info("Columns in collision_data dataset \n %s", [col + " has " + str(len(chicago_collision_data[col].unique())) + " unique values" for col in chicago_collision_data.columns])

    # log the unique value count for the column in flight_call dataframe

    #logger.info("Columns in flight_call dataset \n %s", [col + " has " + str(len(flight_call[col].unique())) + " unique values" for col in flight_call.columns])

        flight_call_clean = flight_call.rename(columns={"Species": "Genus", "Family": "Species"}, errors=False)
   
    # log the unique value count for the column in flight_call_clean dataframe, renaming the columns

    #logger.info("Columns in flight_call_clean dataset \n %s", [col + " has " + str(len(flight_call_clean[col].unique())) + " unique values" for col in flight_call_clean.columns])

     # log the unique value count for the column in light_levels dataframe
    
    #logger.info("Columns in flight_call_clean dataset  \n %s", [col + " has " + str(len(light_levels[col].unique())) + " unique values" for col in light_levels.columns])
